...
import { createProgramNoticeAction } from './actions'
...
export function ProgramNoticeForm({ slug, programId, today, schedules }: Props) {
  const [type, setType] = useState<ProgramNoticeType>('MAKEUP')
  const [title, setTitle] = useState('')
  const [content, setContent] = useState('')
  const [isVisible, setIsVisible] = useState(true)

  const [makeupDate, setMakeupDate] = useState(today)
  ...
  const [changeFromTime, setChangeFromTime] = useState('')
  const [changeToTime, setChangeToTime] = useState('')
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      await createProgramNoticeAction(fd)
    })
  }
